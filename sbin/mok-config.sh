...
read -rp "OpenSSL MOK config file path (default: /etc/ssl/openssl-mok.cnf): " CONFIGFILE
CONFIGFILE="${CONFIGFILE:-"/etc/ssl/openssl-mok.cnf"}"

read -rp "Country code (default: UK): " COUNTRYCODE
COUNTRYCODE="${COUNTRYCODE:-"UK"}"

read -rp "State, region or province code (default: LDN): " STATE
STATE="${STATE:-"LDN"}"

read -rp "City or town (default: London): " CITY
CITY="${CITY:-"London"}"

read -rp "Organisation name (default: Canonical Ltd.): " ORG
ORG="${ORG:-"Canonical Ltd."}"

read -rp "Email address (default: user@ubuntu.com): " EMAIL
EMAIL="${EMAIL:-user@ubuntu.com}"

echo ''
read -rp "Choose the extendedKeyUsage extension (default: 4)
